            ...
            <div class="flex items-center space-x-4">
                ...
                @foreach($menuItems as $item)
                    ...
                    @if(empty($item->children))
                        <a href="{{ $item['url'] }}" class="px-3 py-2 text-sm font-medium rounded-md text-secondary hover:bg-primary/10 hover:text-primary">
                            {{ $item['name'] }}
                        </a>
                    @else
                        <div x-data="{
                            ...
                            <div x-show="open"
                                ...
                                @foreach($item->children as $child)
                                    @if(empty($child->children))
                                        <a href="{{ route($child->route) }}" class="block px-4 py-2 text-sm text-secondary hover:bg-primary/10 dark:text-gray-300 dark:hover:bg-gray-700">
                                            {{ $child->name }}
                                        </a>
                                    @else
                                        <div x-data="{ subOpen: false }"
                                            ...
                                            <button
                                                class="flex items-center justify-between w-full px-4 py-2 text-sm text-secondary hover:bg-primary/10 dark:text-gray-300 dark:hover:bg-gray-700"
                                            >
                                                <span>{{ $child->name }}</span>
                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                    class="w-4 h-4 ml-2 transition-transform -rotate-90"
                                            ...
                                            <!-- Level 3 menu -->
                                            <div x-show="subOpen"
                                            class="absolute left-full top-0 w-48 ml-0.5 bg-white rounded-lg shadow-lg dark:bg-gray-800"
                                            x-transition:enter="transition ease-out duration-200"
                                            x-transition:enter-start="transform opacity-0 scale-95"
                                            x-transition:enter-end="transform opacity-100 scale-100"
                                            x-transition:leave="transition ease-in duration-150"
                                            ...
                                            @foreach($child->children as $subChild)
                                                ...
                                                @else
                                                    <div x-data="{ subSubOpen: false }"
                                                        class="relative"
                                                        @mouseenter="subSubOpen = true"
                                                        @mouseleave="subSubOpen = false">
                                                        <button
                                                            class="flex items-center justify-between w-full px-4 py-2 text-sm text-secondary hover:bg-primary/10 dark:text-gray-300 dark:hover:bg-gray-700"
                                                        >
                                                        ...
                                                        <div x-show="subSubOpen"
                                                            ...
                                                            @foreach($subChild->children as $subSubChild)
                                                                <a href="{{ route($subSubChild->route) }}"
                                                                    class="block px-4 py-2 text-sm text-secondary hover:bg-primary/10 dark:text-gray-300 dark:hover:bg-gray-700">
                                                                    {{ $subSubChild->name }}
                                                                </a>
                                                            @endforeach
                                        ...
                                        @else
                                            <div x-data="{ subOpen: false }" class="relative">
                                                <button
                                                    ...
                                                    class="flex items-center justify-between w-full px-4 py-2 text-sm font-medium text-gray-600 transition-colors duration-300 rounded-lg dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800">
                                                    <span>{{ $child->name }}</span>
                                                    <svg class="w-4 h-4 ml-2 transition-transform duration-200"
                                                ...
                                                <div x-show="subOpen"
                                                    ...
                                                    @foreach($child->children as $subChild)
                                                        @if(empty($subChild->children))
                                                            <a href="{{ route($subChild->route) }}"
                                                                class="flex items-center w-full px-4 py-2 text-sm font-medium text-gray-500 transition-colors duration-300 rounded-lg dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800">
                                                                {{ $subChild->name }}
                                                            </a>
                                                        @else
                                                            <div x-data="{ subSubOpen: false }" class="relative">
                                                                <button
                                                                    @click="subSubOpen = !subSubOpen"
                                                                    class="flex items-center justify-between w-full px-4 py-2 text-sm font-medium text-gray-500 transition-colors duration-300 rounded-lg dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800">
                                                                    <span>{{ $subChild->name }}</span>
                                                                    <svg class="w-4 h-4 ml-2 transition-transform duration-200"
                                                                ...
                                                                <div x-show="subSubOpen"
                                                                    ...
                                                                    @foreach($subChild->children as $subSubChild)
                                                                        <a href="{{ route($subSubChild->route) }}"
                                                                            class="flex items-center w-full px-4 py-2 text-sm font-medium text-gray-400 transition-colors duration-300 rounded-lg dark:text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800">
                                                                            {{ $subSubChild->name }}
                                                                        </a>
                                                                    @endforeach
                                                                </div>
